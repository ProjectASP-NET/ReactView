  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
        <div
          key={i}
          className="h-80 animate-pulse rounded-3xl bg-[var(--card-bg)]"
        />
      ))}
    </div>
  );
...
export default function Catalog() {
  return (
    <main className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
      <div className="mb-12 flex flex-col gap-6 border-b border-[var(--border)] pb-8 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-4xl font-black tracking-tight sm:text-5xl">
            НАШ <span className="text-[var(--text-muted)]">КАТАЛОГ</span>
          </h1>
          <p className="mt-4 text-lg text-[var(--text-secondary)]">
            Премиальные жидкости и девайсы для истинных ценителей.
          </p>
        </div>
        <Suspense fallback={<div className="h-10 w-32 animate-pulse rounded-xl bg-[var(--card-bg)]" />}>
          <Sort />
        </Suspense>
      </div>
