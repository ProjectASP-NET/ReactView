    ...
    <main className="flex min-h-screen flex-col">
      <section className="relative flex h-screen w-full items-center justify-center bg-[url('/banner.png')] bg-cover bg-fixed bg-center bg-no-repeat">
        ...
        <div className="relative z-10 flex flex-col items-center gap-6 px-6 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--card-bg)] px-4 py-1.5 backdrop-blur-md">
            <span className="h-2 w-2 rounded-full bg-green-500 animate-pulse" />
            <span className="text-xs font-medium tracking-widest text-[var(--text-primary)] uppercase">
          ...
          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            ...
            <Link
              href="/about"
              className="rounded-full border border-[var(--border)] bg-[var(--card-bg)] px-8 py-4 text-sm font-bold tracking-wider text-[var(--text-primary)] backdrop-blur-md transition-all hover:bg-[var(--card-hover)] hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] active:scale-95"
            >
              О БРЕНДЕ
            </Link>
          </div>
      ...
      <section className="relative z-20 flex min-h-screen flex-col items-center justify-center bg-[var(--section-bg)] px-6 py-24 text-[var(--text-primary)]">
        <h2 className="mb-12 text-center text-4xl font-bold tracking-tight md:text-6xl">
          ПОЧЕМУ <span className="text-[var(--text-muted)]">D&D LIQUID?</span>
        </h2>
        <div className="grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-3">
          {[
            ...
            <div key={i} className="flex flex-col gap-4 rounded-3xl border border-[var(--card-border)] bg-[var(--card-bg)] p-8 transition-colors hover:bg-[var(--card-hover)]">
              <h3 className="text-xl font-bold tracking-wider">{item.title}</h3>
              <p className="font-light text-[var(--text-secondary)] leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
