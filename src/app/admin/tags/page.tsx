...
export default function AdminTagsPage() {
  ...
  const load = async () => {
    try { setError(null); setLoading(true); setItems(await AdminTagService.getAll()); }
  ...
  const openEdit = (item: TagDTO) => {
    setEditing(item); setForm({ name: item.name }); setShowModal(true);
  };

  const handleSave = async () => {
    try {
      if (editing) await AdminTagService.update(editing.id, form);
      else await AdminTagService.create(form);
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await AdminTagService.delete(id); load(); }
    catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-(--text-primary)">Теги</h1>
      ...
      <div className="relative mb-4">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск тегов..." className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-(--card-bg) border border-(--border) text-(--text-primary) outline-none focus:border-(--accent)" />
      </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-(--border)">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-(--border)">
              ...
              {filtered.length === 0 && <tr><td colSpan={3} className="p-8 text-center text-(--text-muted)">Теги не найдены</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowModal(false)}>
          <div className="bg-(--card-bg) rounded-2xl p-6 w-full max-w-md border border-(--border)" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-bold text-(--text-primary) mb-4">{editing ? "Редактировать" : "Создать"} тег</h2>
            <div>
